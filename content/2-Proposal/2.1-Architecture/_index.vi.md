...
#### 6.1 Tài liệu API thống nhất (31 endpoint)
...
**Admin Operations — 3 endpoint** (bổ sung bởi bản điều chỉnh §6.6; yêu cầu role `admin` qua `cognito:groups`)

| # | Endpoint | Method | Input | Output | Use Case |
| - | -------- | ------ | ----- | ------ | -------- |
| 1 | `/api/v1/admin/courts` | `GET` | Query: `status` (mặc định `PENDING`), `page`, `limit` | `{ data: [...courts], total, page, limit }` | Admin xem hàng chờ các sân mới đăng ký cần review trước khi lên nền tảng. |
| 2 | `/api/v1/admin/courts/{court_id}/review` | `PATCH` | `action` (`approve` / `reject`), `reason` (bắt buộc khi reject) | Bản ghi sân đã cập nhật | Admin chuyển `PENDING` → `ACTIVE` hoặc `REJECTED`; lý do được lưu vào `courts.rejection_reason` và SNS gửi thông báo đến court manager. |
| 3 | `/api/v1/admin/users/{user_id}/role` | `PATCH` | `role` (`player` / `court_manager` / `admin`) | Bản ghi người dùng đã cập nhật | Admin đổi vai trò theo nguyên tắc **Cognito trước**: cập nhật Cognito group (`AdminAddUserToGroup` / `AdminRemoveUserFromGroup`), rồi mới tới cache `users.role` — xem §6.6. |

**Manager Analytics — 1 endpoint** (bổ sung bởi bản điều chỉnh §6.6; yêu cầu role `court_manager`)

| # | Endpoint | Method | Input | Output | Use Case |
| - | -------- | ------ | ----- | ------ | -------- |
| 1 | `/api/v1/manager/revenue` | `GET` | Query: `date_from`, `date_to`, `court_id?`, `group_by?` (`day` / `court`) | `{ total_revenue, currency, bookings_count, breakdown? }` | Doanh thu cho dashboard: `SUM` tại tầng DB trên **`payments` có `status = 'SUCCESS'`** join với các sân của người gọi (giới hạn theo `owner_id` — quy tắc chống IDOR); `group_by` phục vụ biểu đồ. Lý do chọn cách tính xem §6.6. |

**User Profile — 2 endpoint** (bổ sung bởi bản điều chỉnh §6.6; mọi người dùng đã xác thực — hoãn triển khai, ưu tiên thấp)

| # | Endpoint | Method | Input | Output | Use Case |
| - | -------- | ------ | ----- | ------ | -------- |
| 1 | `/api/v1/users/me` | `GET` | Bearer token (header) | Hồ sơ người dùng | Hồ sơ của chính người dùng hiện tại (tên, SĐT, avatar, vai trò). Mẫu `me` (như `/bookings/me`) lấy danh tính từ JWT — không có tham số user-ID để bị khai thác. |
| 2 | `/api/v1/users/me` | `PUT` | Bất kỳ: `full_name`, `phone`, `avatar_url` | Hồ sơ đã cập nhật | Người dùng tự cập nhật thông tin cá nhân sau khi đăng ký. |
...
**`courts`** (điều chỉnh): các giá trị `status` trở thành `PENDING` / `ACTIVE` / `INACTIVE` / `MAINTENANCE` / `REJECTED` — sân do manager đăng ký mặc định `PENDING` cho tới khi admin phê duyệt; tìm kiếm công khai chỉ trả về sân `ACTIVE`. Bản điều chỉnh §6.6 bổ sung cột `rejection_reason VARCHAR(500)` (nullable) để lý do từ chối còn lưu lại sau thông báo SNS và hiển thị được trong dashboard của manager.
...
| Phạm vi API            | 4 endpoint thanh toán                      | **31 endpoint** trải khắp xác thực, đặt sân, quản lý sân, thanh toán, admin và analytics |
...
#### 6.6 Bản điều chỉnh thiết kế (19/07/2026) — Admin Operations & Manager Analytics

Do **Nguyên** đề xuất và được review trong buổi họp nhóm Tuần 5: sáu endpoint mở rộng §6.5, lấp hai lỗ hổng còn bỏ ngỏ. Được chấp nhận với các điều chỉnh bên dưới; API tăng từ 25 → **31 endpoint**.

**Lỗ hổng 1 — vòng duyệt sân không có người vận hành.** §6.5 cho sân do manager đăng ký bắt đầu ở `PENDING`, nhưng không endpoint nào chuyển được sân ra khỏi trạng thái đó: manager chỉ thao tác được trên sân đã duyệt, còn người chơi không bao giờ thấy sân chưa `ACTIVE`. Cặp endpoint hàng chờ + review của admin hoàn thiện máy trạng thái, kèm thông báo SNS đến manager với mỗi quyết định. Lý do từ chối được **lưu lại** (`courts.rejection_reason`) thay vì chỉ nằm trong thông báo — thứ gì người dùng có thể cần đọc lại thì thuộc về bảng dữ liệu, không phải tin nhắn.

**Lỗ hổng 2 — dashboard của manager chưa có API về dòng tiền.** Endpoint doanh thu tổng hợp từ **`payments` có `status = 'SUCCESS'`**, không dùng tổng tiền booking, vì hai bảng trả lời hai câu hỏi khác nhau — *giá đã niêm yết* và *tiền thực sự đang giữ*:

| Tình huống | Tổng tiền booking | Thực tế dòng tiền | SUM theo `payments` |
| --- | --- | --- | --- |
| Đã xác nhận và chơi (`COMPLETED`) | tính | đang giữ ✅ | tính ✅ |
| `NO_SHOW` — đã trả tiền, không đến | tính | đang giữ ✅ | tính ✅ |
| `CANCELLED`, đã hoàn tiền | tính ❌ | đã trả lại | loại ✅ (`REFUNDED`) |
| `PENDING`, chưa từng thanh toán | tính ❌ | chưa từng có | loại ✅ |

Việc xử lý hoàn tiền đúng *ngay từ cấu trúc* — không cần nhánh đặc biệt nào. Phạm vi dữ liệu theo quy tắc sở hữu của §6.5 (join qua `courts.owner_id = người gọi`), và `group_by=day|court` giúp một endpoint phục vụ cả con số tổng lẫn biểu đồ, giữ phần tổng hợp ở tầng Postgres.

**Đổi vai trò theo nguyên tắc Cognito trước.** `users.role` chỉ là cache; nguồn chân lý là Cognito group (§6). Endpoint đổi vai trò phải gọi `AdminAddUserToGroup`/`AdminRemoveUserFromGroup` **trước khi** cập nhật bản ghi DB — nếu chỉ cập nhật DB, nhãn trên UI đổi nhưng mọi kiểm tra phân quyền vẫn đọc claim cũ trong JWT. Lưu ý: JWT đã cấp vẫn giữ `cognito:groups` cũ cho tới khi refresh (~15 phút), nên việc thăng vai trò có hiệu lực ở lần đăng nhập/refresh kế tiếp — là hành vi bình thường, không phải lỗi.

**Chuẩn hóa cách đặt tên.** Các endpoint hồ sơ dùng `GET/PUT /users/me` (khớp `/bookings/me`) thay cho URL kiểu động từ: tài nguyên nằm trong path, hành động nằm ở method — và mẫu `me` loại bỏ hoàn toàn tham số user-ID, tức là loại bỏ luôn bề mặt tấn công IDOR đó. Phần hồ sơ **hoãn triển khai** (ưu tiên thấp).

**Ảnh hưởng lan tỏa:** một migration Alembic (`courts.rejection_reason`, nullable — tương thích ngược); cập nhật ERD (Mermaid + drawio); frontend thêm màn hình review cho admin và biểu đồ doanh thu trong dashboard của manager; endpoint đổi vai trò cần backend có quyền IAM/SDK gọi các API admin của Cognito.
